server = function(input, output, session) {
  ...
  lyr_names <- eventReactive(input$selectsource, {
    if (!is.null(input$upload_intact)) {
    ...
    } else if (input$selectsource == "usedemo"){
  ...
  stream_sf <- reactive({
    ...
    req(input$previewLayers)
    if(input$selectsource == "usedemo"){
      return(st_read("www/demo.gpkg", 'streams', quiet=T))
    } else if (!is.null(input$csv_paths)) {
      req(validate_csv())
      return(read_shp_from_csv(input$csv_paths, "streams"))
      ...
      return(st_read(input$gpkg_file$datapath, input$streams_layer, quiet = TRUE))
    }else {
      return(NULL)
    }
  })
 ...
 # })
  ...
  #  if(!is.null(input$upload_aoi)){
  #    catch_sparse <- st_intersects(st_centroid(catch_pr), aoi_sf())
  #    catch_int <- which(lengths(catch_sparse) > 0)
  #    catch_sf <- catch_pr[catch_int, ]
  ...
  catch_stem <- eventReactive(input$confAnalysis, {
    ...
    {# show pop-up ...
      ...
      catch_stem <- catchment_pr()[catchment_pr()$CATCHNUM %in% downstream_stem_list$AOI_1, drop = TRUE]
      catch_stem$stem <- 1
      merge(catchment_pr(), catch_stem[,c("CATCHNUM", "stem")], by = "CATCHNUM", all.x = TRUE)
    }
  })
  ...
  # DOWNSTREAM SECTION
  ####################################################################################################
  catch_att <- eventReactive(input$confAnalysis, {
    if(is.null(aoi_sf()))
    { # show pop-up ...
      showModal(modalDialog(
    ...
    {# show pop-up ...
      ...
      catch_down<- catchment_pr()[catchment_pr()$CATCHNUM %in% catchList$AOI_1, drop = TRUE]
      catch_down$down <- 1
      # Final dataframe
  ...
  observe({
    ...
    leafletProxy("map") %>%
      clearGroup("Catchmentss AOI") %>%
      addPolygons(data = data_select, color='black', weight = 1, fillColor = "grey", fillOpacity = 0.7,
                  layerId = data_select$CATCHNUM, group = "Catchmentss AOI") %>%
      showGroup("Catchmentss AOI")
  })
  ...
  observeEvent(input$editAOI,{
    ...
    leafletProxy("map") %>%
      fitBounds(map_bounds1[1], map_bounds1[2], map_bounds1[3], map_bounds1[4]) %>%
      #addPolygons(data=catch_aoi, fill = F, color="red", weight=4, group="Catchments AOI", options = leafletOptions(pane = "ground")) %>%
  ...
  observeEvent(input$confAOI, {
    ...
    legend <- c(overlayBase(),  "AOI", "Catchments AOI")
    overlayBase(legend)
    ...
    leafletProxy("map") %>%
      ...
      clearGroup('Selected') %>%
      fitBounds(map_bounds1[1], map_bounds1[2], map_bounds1[3], map_bounds1[4]) %>%
      addPolygons(data=analysis_aoi, fill = F, color="red", weight=4, group="Catchments AOI", options = leafletOptions(pane = "ground")) %>%
      addPolygons(data=aoi, fill=F, color="black", weight=3, group="AOI", options = leafletOptions(pane = "ground")) %>%
      addLayersControl(overlayGroups = c(overlayBase(), grps()),
                       options = layersControlOptions(collapsed = FALSE)) %>%
      hideGroup(c("Fires", "Protected areas (PAs)", "Intactness"))
    showModal(modalDialog(
      title = "Catchments AOI displayed.",
      easyClose = TRUE,
      footer = modalButton("OK")))
  })
  ...
  observe({
    ...
    catch_att <- catch_att %>% 
      filter(down==1) 
    downstream_sf(catch_att)
    
    catch_stem <- catch_att %>% 
      filter(stem ==1) 
    downstream_stem_sf(catch_stem)
    ...
    leafletProxy("map") %>%
      ...
      clearControls() %>%
      fitBounds(map_bounds1[1], map_bounds1[2], map_bounds1[3], map_bounds1[4]) %>%
      addPolygons(data=catch_att, color=~catchintact(intact), stroke=F, fillOpacity=0.8, group='Downstream area', options = leafletOptions(pane = "ground")) %>%
      addPolygons(data=catch_stem, color=~catchintact(intact), stroke=F, fillOpacity=0.8, group="Downstream stem", options = leafletOptions(pane = "ground")) %>%
    ...
    if(nrow(catch_up)>0){
      leafletProxy("map") %>%
        addPolygons(data=catch_up, color=~catchintact(intact), stroke=F, fillOpacity=0.8, group="Upstream area", options = leafletOptions(pane = "ground")) %>%
        addLayersControl(position = "topright",
                         baseGroups=c("Esri.WorldTopoMap", "Esri.WorldImagery"),
  ...
  observeEvent(input$confAOI, {
    ...
    x <- x %>% 
      mutate(Area_km2 = case_when(Variables == "Study area" ~  round(as.numeric(st_area(planreg_sf())/1000000,0))),
             Percent= case_when(Variables == "Study area" ~  100),
             Area_km2 = case_when(Variables == "Catchments AOI" ~  round(as.numeric(st_area(st_union(analysis_aoi()))/1000000,0)), 
                                  TRUE ~ Area_km2),
             Percent= case_when(Variables == "Catchments AOI" ~ round(as.numeric(st_area(st_union(analysis_aoi()))/st_area(planreg_sf())*100,2)),
                                Variables == "Catchments AOI intactness" ~ round(as.numeric(sum(analysis_aoi()$intact*st_area(analysis_aoi())))/as.numeric(st_area(st_union(analysis_aoi())))*100,2),
                                TRUE ~ Percent)
      )
    outtab1(x)
    ...
    if(!is.null(fire_sf())){
      ...
      new_firerows <- tibble(Variables=c("Within Catchments AOI"), 
                             Area_Burned_km2= NA_real_,
                             `Area_Burned_%` = NA_real_)
      ...
      if(nrow(fire_aoi)>0){
        y <- y %>% 
          mutate(Area_Burned_km2 = case_when(Variables == "Within Catchments AOI" ~  round(as.numeric(sum(st_area(fire_aoi))/1000000,2)), 
                                             TRUE ~ Area_Burned_km2),
                 `Area_Burned_%`= case_when(Variables == "Within Catchments AOI" ~ round(as.numeric(sum(st_area(fire_aoi))/sum(st_area(analysis_aoi())))*100,2),
                                            TRUE ~ `Area_Burned_%`))
      }else{
        y <- y %>% 
          mutate(Area_Burned_km2 = case_when(Variables == "Within Catchments AOI" ~  0, 
                                             TRUE ~ Area_Burned_km2),
                 `Area_Burned_%`= case_when(Variables == "Within Catchments AOI" ~  0, 
                                            TRUE ~ `Area_Burned_%`)
          )
      }
      outfiretab(y)
