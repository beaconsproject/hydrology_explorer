...
ui = dashboardPage(skin="blue",
    ...
    dashboardSidebar(
     ...
     sidebarMenu(id = "tabs",
       ...
       menuItem("Select study area & AOI", tabName = "select", icon = icon("th")),
       menuItem("Generate upstream and downstream", tabName = "upstream", icon = icon("th")),
       hr()
     ),
     conditionalPanel(
       condition="input.tabs=='select'",
       #h4(" Choose intactness map and study area from one of the two options:"),
       HTML("<h4>&nbsp; &nbsp; Choose intactness map and study &nbsp; &nbsp; &nbsp; area from one of the two options:</h4>"),
       selectInput("select_fda", label="Use default FDA intactness map (disturbances buffered 500m)", choices=c("Select an FDA", fda_list), selected="Select an FDA"),
       #actionButton("distButton", "Use default disturbance"),
       fileInput(inputId = "upload_dist", label = "Upload .gpkg generated by the Regional Disturbance Explorer app (intactness and footprint maps):", multiple = FALSE, accept = ".gpkg")
     ),
    conditionalPanel(
      condition="input.tabs=='select'",
      #h4(" Choose Area of Interest (AOI) from one of the two options:"),
       ...
       tabItem(tabName="overview",
               fluidRow(
                 tabBox(id = "one", width="8",
                   tabPanel(HTML("Welcome!"), includeMarkdown("docs/overview.md")),
