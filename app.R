...
ui = dashboardPage(skin="blue",
    ...
    dashboardSidebar(
     ...
     sidebarMenu(id = "tabs",
       ...
       menuItem("Select study area", tabName = "selectSA", icon = icon("th")),
       menuItem("Select AOI", tabName = "selectAOI", icon = icon("th")),
       menuItem("Generate upstream and downstream", tabName = "upstream", icon = icon("th")),
       menuItem("Download results", tabName = "download", icon = icon("th")),
       hr()
     ),
     conditionalPanel(
       condition="input.tabs=='selectSA'",
       HTML("<h4>&nbsp; &nbsp; Choose intactness map and study &nbsp; &nbsp; &nbsp; area from one of the two options:</h4>"),
       selectInput("select_fda", label= div(style = "font-size:13px","Use default FDA intactness map (disturbances buffered 500m)"), choices=c("Select an FDA", fda_list), selected="Select an FDA"),
       #actionButton("distButton", "Use default disturbance"),
       fileInput(inputId = "upload_dist", label = div(style = "font-size:13px", "Upload .gpkg generated by the Regional Disturbance Explorer app (intactness and footprint maps):"), multiple = FALSE, accept = ".gpkg")
     ),
    conditionalPanel(
      condition="input.tabs=='selectAOI'",
      HTML("<h4>&nbsp; &nbsp; Choose Area of Interest (AOI) from &nbsp; &nbsp; one of the two options:</h4>"),
      fileInput(inputId = "upload_poly", label = div(style = "font-size:13px", "Upload a polygon:"), multiple = FALSE, accept = ".gpkg"),
      div(style = "margin: 15px; margin-top: -30px; font-size:13px;font-weight: bold", "Or"),
      div(style = "margin: 15px; font-size:13px; font-weight: bold", "Select a set of catchments on the map"),
      #actionButton(inputId = "gen_aoi_button", label = div(style = "font-size:13px","Generate AOI using catchments"), icon = icon(name = "circle-plus", lib = "font-awesome"), style="width:200px"),
      actionButton(inputId = "gen_aoi_button", label = div(style = "font-size:13px",HTML("Generate AOI using the <br/> selected set of catchments")), style="width:200px"),
      tags$br(),
      tags$br()
    ),
     conditionalPanel(
       ...
       tabItem(tabName="overview",
               fluidRow(
                 tabBox(id = "one", width="8",
                   tabPanel(HTML("<b>Mapview</b>"), leafletOutput("map", height=750) %>% withSpinner()),
                 ...
                 tabBox(id = "two", width="4",
                        tabPanel(HTML("<b>Upstream and downstream statistics</b>"), tableOutput("tab1"),
...
server = function(input, output) {
  ...
  catchments <- reactive({
    #st_read('www/fda10.gpkg', 'catchments_yk', quiet=T)
    st_read('www/catchments_yt.shp', quiet=T)
  ...
  streams <- reactive({
    #st_read('www/fda10.gpkg', 'stream_3578', quiet=T)
    st_read('www/stream_yt_1M.shp', quiet=T)
  ...
  catch_up <- eventReactive(input$goButtonDown, {
    ...
    if(nrow(upstream_list)==0)
    {
      ...
      catch_up <- catchment()[catchment()$CATCHNUM %in% upstream_list$AOI_01,]
      catch_up <- catch_up %>%
        mutate(up =1) %>%
        st_drop_geometry()
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      ...
      addPolylines(data=stream_4326, color='#0066FF', weight=1, group="Streams") %>%
  ...
  observe({
    ...
    addPolygons(data=planreg, color='red', fill=F, weight=4, group='Study area', options = leafletOptions(pane = "layer1")) %>%
    ...
    addLayersControl(position = "topright",
                     baseGroups=c("Esri.NatGeoWorldMap", "Esri.WorldImagery"),
                     overlayGroups = c("Data extent", "FDAs", "Study area", "Catchments", "Streams", "Intactness", "Footprint"),
                     options = layersControlOptions(collapsed = FALSE))  %>%
  ...
  observe({
    ...
    clearGroup('Streams') %>%
    fitBounds(map_bounds1[1], map_bounds1[2], map_bounds1[3], map_bounds1[4]) %>%
    addPolygons(data=planreg, color='red', fill=F, weight=4, group='Study area') %>%
    addPolygons(data=catch_4326, color='black', fillColor = "grey", fillOpacity = 0, weight=1, layerId = catch_4326$CATCHNUM, group="Catchments", options = leafletOptions(pane = "layer2")) %>%
    addPolygons(data=intact, color='darkgreen', stroke=F, fillOpacity=0.5, group='Intactness', options = leafletOptions(pane = "layer1")) %>%
    addPolygons(data=footprint, fill=T, stroke=F, fillColor='black', fillOpacity=0.5, group="Footprint", options = leafletOptions(pane = "layer2")) %>%
    addPolylines(data=stream_4326, color='#0066FF', weight=1.2, group="Streams", options = leafletOptions(pane = "layer1")) %>%
    ...
    addLayersControl(position = "topright",
                       baseGroups=c("Esri.NatGeoWorldMap", "Esri.WorldImagery"),
                       overlayGroups = c("Data extent", "FDAs", "Study area","Streams", "Catchments", "Intactness", "Footprint"),
                       options = layersControlOptions(collapsed = FALSE))%>%
      hideGroup(c("Intactness"))
  })  
  ...
  observe({
    ...
    leafletProxy("map") %>%
        ...
        addPolygons(data=intact, color='darkgreen', stroke=F, fillOpacity=0.5, group='Intactness', options = leafletOptions(pane = "layer1")) %>%
        addPolygons(data=footprint, fill=T, stroke=F, fillColor='black', fillOpacity=0.5, group="Footprint", options = leafletOptions(pane = "layer2")) %>%
        addLayersControl(overlayGroups = c("Data extent", "FDAs", "Study area", "Catchments", "Streams",  "Intactness", "Footprint", "AOI"),
                       options = layersControlOptions(collapsed = FALSE)) %>%
        hideGroup(c("Intactness"))
    })
  ...
  observe({
    ...
    #clearGroup('Footprint') %>%
    fitBounds(map_bounds1[1], map_bounds1[2], map_bounds1[3], map_bounds1[4]) %>%
    addPolygons(data=catch_att, color=~catchintact(intact), stroke=F, fillOpacity=0.8, group='Downstream area', options = leafletOptions(pane = "layer1")) %>%
    ...
    addPolylines(data=stream_4326, color='#0066FF', weight=2, group="Streams", options = leafletOptions(pane = "layer1")) %>%
    ...
    addLayersControl(position = "topright",
                     baseGroups=c("Esri.NatGeoWorldMap", "Esri.WorldImagery"),
                     overlayGroups = c("Data extent", "Study area", "AOI", "Catchments", "Streams","Footprint", "Downstream area", "Downstream stem"),
                     options = layersControlOptions(collapsed = FALSE)) %>%
    ...
    if(nrow(catch_up)>0){
      ...
      addLayersControl(position = "topright",
                         baseGroups=c("Esri.NatGeoWorldMap", "Esri.WorldImagery"),
                         overlayGroups = c("Data extent", "Study area", "AOI", "Catchments", "Streams","Footprint", "Upstream area", "Downstream area", "Downstream stem"),
                         options = layersControlOptions(collapsed = FALSE)) %>%
        hideGroup(c("Downstream stem"))
    } 
  })
  ...
  outputtab1 <- reactive({
    ...
    if(input$goButtonDown > 0) {
      ...
      x$Area_km2[x$Variables=="Study area"] <- round(st_area(planreg_sf())/1000000,0)
      x$Area_km2[x$Variables=="AOI"] <- round(st_area(st_union(aoi_sf()))/1000000,0)
      x$Percent[x$Variables=="Percent AOI in study area"] <- round(st_area(st_union(aoi_sf()))/st_area(planreg_sf())*100,2)
      ...
      x$Percent[x$Variables=="Downstream mean AWI*"] <- round(((sum(downstream_int$Area_total)-sum(downstream_int$area_dist))/sum(downstream_int$Area_total))*100,1)
    }else{
      req(aoi_sf())
      x <- tibble(Variables=c("Study area",
                              "AOI",
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      catch_att <- catch_att()[,c("CATCHNUM", "Area_land", "Area_water", "Area_total", "FDA", "area_dist", "intact", "down", "stem", "up")]
