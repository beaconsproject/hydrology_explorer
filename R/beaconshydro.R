...
calc_dci <- function(conservation_area_sf, stream_sf, buffer_width = 0.1){
  ...
  reserve_dci <- 
    sf::st_intersection(stream_sf, sf::st_union(conservation_area_sf)) %>% # get streams just for the required region
    ...
    #sf::st_make_valid(.data, model = "closed")%>%
    sf::st_intersection(conservation_area_sf, .data) %>% # intersect with reserves to get buffered stream for each reserve
  ...
  dci <- dplyr::left_join(sf::st_drop_geometry(conservation_area_sf), reserve_dci, by = "network") %>%
